...
from config import BOT_TOKEN, ADMIN_CHAT_ID, ADMIN_IDS, DB_PATH, CHAT_ID  # Добавлен CHAT_ID
...
async def main():
    ...
    await bot.set_my_commands([
        ...
        types.BotCommand(command="topfast", description="Принудительный топ (Админ)"),  # Новая команда
    ])
...
async def scheduler_loop(bot: Bot):
    ...
    # Создаем временную зону МСК (UTC+3)
    msk_tz = timezone(timedelta(hours=3))
    
    last_inactive_check = datetime.now(msk_tz) - timedelta(hours=1)
    last_daily_reset = datetime.now(msk_tz) - timedelta(days=1)
    last_weekly_reset = datetime.now(msk_tz) - timedelta(days=7)
    last_redemption_check = datetime.now(msk_tz) - timedelta(hours=1)
    
    while True:
        try:
            # Все расчеты времени теперь строго по МСК
            ...
            # Ежедневный сброс — если прошло больше 23 часов, и сейчас 00:00-00:05 по МСК
            if (now - last_daily_reset).total_seconds() >= 82800:  # 23 часа
                if now.hour == 0 and now.minute <= 5:
                    last_daily_reset = now
                    await daily_reset_task(bot)
            
            # Еженедельный сброс — если прошло больше 6 дней, и сейчас воскресенье 23:00-23:05 по МСК
...
async def daily_reset_task(bot: Bot):
    ...
    if winners:
        ...
        # Результат отправляется в ОСНОВНОЙ чат
