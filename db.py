    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                last_inactive_notification TEXT  -- ← ДОБАВЛЕНО
            )
        """)
...
async def get_top_streak(limit: int = 15) -> List[Dict]:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        for row in rows:
            data = dict(row)
            # Если имя не задано — используем telegram_username или user_id
...
async def start_redemption(user_id: int, lost_streak: int):
    ...
    # В тестовом режиме 24 часа = 24 секунды
    if is_test_mode():
...
# ============================================================
# 10. УВЕДОМЛЕНИЯ О НЕАКТИВЕ (НОВОЕ)
# ============================================================
