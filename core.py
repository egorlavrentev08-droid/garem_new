...
async def process_user_message(user_id: int, chat_id: int, username: str = None, tg_name: str = None) -> Dict:
    ...
    if is_main_chat:
        ...
        # Логика прогрессивного начисления стриков за день (100 -> +1, 1000 -> еще +1)
        if last_activity and messages_today >= 1000 and streak_awarded_today < 2:
            increment_amount = 2 - streak_awarded_today
            new_award_level = 2
            should_increment = True
        elif last_activity and messages_today >= 100 and streak_awarded_today < 1:
            increment_amount = 1
            new_award_level = 1
            should_increment = True
        ...
        if should_increment:
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                await db.execute(
                    "UPDATE users SET streak_awarded_today = ? WHERE user_id = ?",
                    (new_award_level, user_id)
                )
                await db.commit()
        else:
            new_streak = user['streak']
    ...
    rank_frozen = user.get('rank_frozen', 0)
    
    # Автоматическое изменение рангов происходит ТОЛЬКО если ранг не заморожен админом
    if rank_frozen == 0 and is_main_chat:
    ...
    if should_increment and is_main_chat:
        # Кастомный текст для преодоления планки в 1000 сообщений
        if new_award_level == 2 and increment_amount == 1:
            response_data['text'] = "🔥 Ого! Ты набрал 1000 сообщений за сегодня! За такую мега-активность твой стрик увеличивается ещё на +1 день! КРАСАВА! 🦊"
            return response_data
        elif new_award_level == 2 and increment_amount == 2:
            response_data['text'] = "🔥 Невероятная активность! Ты набрал более 1000 сообщений за день и получаешь сразу +2 дня к стрику! 🦊"
            return response_data
...
async def get_user_profile(user_id: int) -> Optional[str]:
    ...
    # Добавим визуальный индикатор заморозки в профиль для информативности
    freeze_status = " ❄️ (Заморожен)" if user.get('rank_frozen', 0) else ""
    
    text = (
        f"📋 **Профиль**\n"
        f"| Имя: {user['name'] or 'Без имени'}\n"
        f"| Ранг: {user['rank']}{freeze_status}\n"
...
async def admin_set_rank(admin_id: int, identifier: str, rank_name: str) -> Tuple[bool, str]:
    ...
    # Проверка команды разморозки / сброса
    if rank_name.lower() == 'сброс':
        from config import get_rank_by_streak
        new_rank = get_rank_by_streak(user['streak'])
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute(
                "UPDATE users SET rank = ?, rank_frozen = 0 WHERE user_id = ?", 
                (new_rank['name'], user['user_id'])
            )
            await db.commit()
        name = user.get('name') or user.get('tg_name') or user.get('telegram_username') or identifier
        return True, f"✅ Заморозка ранга снята для {name}. Установлен текущий автоматический ранг: {new_rank['name']}."
    
    valid_ranks = [r['name'] for r in RANKS]
    if rank_name not in valid_ranks:
        return False, f"❌ Неверный ранг. Доступны: {', '.join(valid_ranks)} или 'сброс'"
    
    # Установка ручного ранга и его заморозка (rank_frozen = 1)
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE users SET rank = ?, rank_frozen = 1 WHERE user_id = ?", 
            (rank_name, user['user_id'])
        )
        await db.commit()
    
    name = user.get('name') or user.get('tg_name') or user.get('telegram_username') or identifier
    return True, f"✅ Ранг '{rank_name}' установлен для {name} (ID: {user['user_id']}) и заморожен от авто-изменений."
