...
async def process_user_message(user_id: int, chat_id: int, username: str = None, tg_name: str = None) -> Dict:
    ...
    if is_main_chat:
        ...
        if last_activity and streak_awarded_today == 0 and messages_today >= 100:
            should_increment_first = True
        elif last_activity and streak_awarded_today == 1 and messages_today >= 500:
            should_increment_second = True
        
        if should_increment_first or should_increment_second:
            new_streak = await increment_streak(user_id)
            new_val = 1 if should_increment_first else 2
    ...
    is_rank_locked = user.get('rank_locked', 0) == 1
    
    if not is_rank_locked and new_rank['name'] != current_rank and new_rank['name'] != 'Без ранга' and is_main_chat:
    ...
    if (should_increment_first or should_increment_second) and is_main_chat:
        if return_trigger:
...
async def get_top_streak_text(limit: int = 15) -> str:
    ...
    text += "\n✨ Награды в воскресенье:\n🥇 1000 | 🥈 1000 | 🥉 1000 | 4️⃣ 1000 | 5️⃣ 1000 монет"
    return text
...
async def admin_set_rank(admin_id: int, identifier: str, rank_name: str) -> Tuple[bool, str]:
    ...
    if rank_name.lower() == 'сброс':
        ...
        auto_rank = get_rank_by_streak(user['streak'])['name']
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute("UPDATE users SET rank = ?, rank_locked = 0 WHERE user_id = ?", (auto_rank, user['user_id']))
            await db.commit()
        name = user.get('name') or user.get('tg_name') or user.get('telegram_username') or identifier
        return True, f"✅ Фиксированный ранг для {name} сброшен. Автоматический: {auto_rank}"
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE users SET rank = ?, rank_locked = 1 WHERE user_id = ?", (rank_name, user['user_id']))
        await db.commit()
    
    name = user.get('name') or user.get('tg_name') or user.get('telegram_username') or identifier
    return True, f"✅ Ранг '{rank_name}' зафиксирован для {name} (ID: {user['user_id']})"


async def execute_daily_top(bot, is_forced: bool = False):
    from db import award_daily_top, reset_daily_messages
    from config import CHAT_ID
    
    winners = await award_daily_top()
    if winners:
        medals = ["🥇", "🥈", "🥉"]
        title = "ДОСРОЧНО" if is_forced else "ЗА ДЕНЬ"
        text = f"📊 **ТОП СООБЩЕНИЙ {title}**\n\n"
...
async def execute_weekly_top(bot, is_forced: bool = False):
    from db import award_weekly_top
    from config import CHAT_ID
    
    winners = await award_weekly_top()
    if winners:
        medals = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣"]
        title = "ДОСРОЧНО" if is_forced else "ЕЖЕНЕДЕЛЬНЫЙ"
        text = f"🏆 **{title} ТОП СТРИКОВ**\n\n"
