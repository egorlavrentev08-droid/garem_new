...
_phrase_cache = {}
_last_rank_notified = {}  # Защита от спама рангами
...
def get_default_mood_for_trigger(trigger: str) -> str:
    """Возвращает дефолтное настроение для триггера"""
    defaults = {
...
async def process_user_message(user_id: int, chat_id: int) -> Dict:
    ...
    # 2. Обновляем активность и счётчик сообщений
    ...
    # ============================================================
    # 5. Увеличиваем стрик ЕСЛИ:
    #    - Сегодня ещё не начисляли стрик
    #    - User написал >= 100 сообщений за сегодня
    #    - И это НЕ первое сообщение в жизни (проверяем last_activity)
    # ============================================================
    should_increment = False
    now = datetime.now()
    
    # Получаем сегодняшний счётчик сообщений
    messages_today = user.get('messages_today', 0)
    
    # Проверяем, было ли уже сообщение сегодня
    if last_activity:
        try:
            last_time = datetime.fromisoformat(last_activity)
            # Если последняя активность была СЕГОДНЯ или ВЧЕРА
            if last_time.date() == now.date():
                # Сегодня уже было сообщение — проверяем счётчик
                if messages_today >= 100:
                    should_increment = True
            elif last_time.date() < now.date():
                # Вчера или раньше — проверяем счётчик
                if messages_today >= 100:
                    should_increment = True
        except:
            pass
    else:
        # Первое сообщение в жизни — НЕ ДАЁМ СТРИК (нужно 100 сообщений)
        should_increment = False
    
    if should_increment:
        new_streak = await increment_streak(user_id)
        # Отмечаем, что сегодня стрик уже начислен
        ...
        new_streak = user['streak']  # Оставляем текущий
    
    # 6. Начисляем монеты (за каждое сообщение!)
    rank = get_rank_by_streak(new_streak)
    income = rank['income']
    await add_coins(user_id, income)
    
    # 7. Проверяем ранг (ОТПРАВЛЯЕМ В ЛС!)
    ...
    # 8. Проверяем достижения стрика (ОТПРАВЛЯЕМ В ЛС!)
    ...
    # 9. Ежедневное приветствие (В ЧАТ!) — только если стрик увеличился
    if should_increment:
    ...
    # 10. Если стрик НЕ увеличился — молчим (не спамим)
    return response_data
...
async def handle_inactive_user(user_id: int, trigger: str, mood: str) -> Dict:
    ...
    Возвращает данные для отправки в ЛС.
    """
    user = await get_user(user_id)
    if not user:
        return {'text': None}
