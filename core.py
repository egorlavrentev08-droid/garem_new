...
from db import (
    ...
    add_reward_history,
    update_inactive_notification, get_inactive_notification_time  # ← ДОБАВЛЕНО
)
...
async def process_user_message(user_id: int, chat_id: int) -> Dict:
    ...
    if last_activity:
        try:
            ...
            # В тестовом режиме время ускорено
    ...
    new_streak = user['streak']  # По умолчанию
    if should_increment:
...
# ============================================================
# 4. ПРОВЕРКА НЕАКТИВА (ПЕРЕПИСАНА)
# ============================================================
...
async def check_inactive_users() -> List[Dict]:
    ...
    for user in inactive:
        ...
        if hours_since >= 48:
            if not last_notif:
                # Первое агрессивное уведомление
                result.append({
                    'user': user,
                    'type': 'aggressive',
                ...
                # Отправляем каждые 3-5 часов (рандомно)
        ...
        if 24 <= hours_since < 48:
            if not last_notif:
                # Первое уведомление уже отправлено выше, пропускаем
                continue
            else:
                hours_since_last = get_time_since_hours(last_notif)
                if hours_since_last >= random.uniform(3, 5):
...
async def handle_inactive_user(user_id: int, trigger: str, mood: str) -> Dict:
    ...
    # Если стрик уже 0 — не сбрасываем повторно
    if user['streak'] == 0:
        return {'text': None}
