...
# Добавлены новые аргументы для передачи данных профиля в БД
async def process_user_message(user_id: int, chat_id: int, username: str = None, tg_name: str = None) -> Dict:
    ...
    # Всегда вызываем регистрацию, чтобы актуализировать Имя / Фамилию и юзернейм в базе
...
async def get_top_streak_text(limit: int = 15) -> str:
    ...
    for i, user in enumerate(top):
        ...
        name = user['display_name']  # Использует подготовленное имя с учетом приоритетов
