...
async def chat_filter(message: types.Message) -> bool:
    ...
    if message.chat.id not in [CHAT_ID, ADMIN_CHAT_ID]:
        # Если это ЛС — проверяем регистрацию
    ...
    # 2. Основной чат — всегда пропускаем (авто-регистрация)
    return True
...
async def handle_all_messages(message: types.Message):
    ...
    if message.chat.type == "private":
        reply_markup = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🗣 В чат", url=f"https://t.me/Gar3mDi")]
        ])
    ...
    # Если картинка/мем не загрузились — отправляем только текст
...
async def cmd_admin_coins(message: types.Message):
    ...
    # Определяем пользователя и сумму
    if parts[1].startswith('@'):
        ...
        # Если не указан пользователь — используем ответ
    ...
    # Если сумма отрицательная — забираем
...
async def cmd_admin_addphrase(message: types.Message):
    ...
    # Сохраняем в глобальной переменной (костыль, но работает)
...
async def handle_phrase_confirm(callback: types.CallbackQuery):
    """Подтверждение добавления фразы"""
    global _pending_phrase
...
async def cmd_admin_delphrase(message: types.Message):
    """/delphrase — удалить фразу (по номеру или тексту)"""
    if not is_admin(message.from_user.id):
        return
