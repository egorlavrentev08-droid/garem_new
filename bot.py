...
# ============================================================
# 2. ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ ДЛЯ УДАЛЕНИЯ
# ============================================================
...
async def handle_all_messages(message: types.Message):
    ...
    if response.get('send_to_dm', False):
        try:
            ...
            # В ЛС не удаляем (чтобы пользователь мог прочитать)
    ...
    # ============================================================
    # В ЧАТ — ТОЛЬКО С ВЕРОЯТНОСТЬЮ 20% И С @USERNAME
    # ============================================================
    
    # 20% шанс ответить в чат (чтобы не спамить)
    if random.random() > 0.2:
        return  # Молчим в чате
    ...
    # ============================================================
    # УДАЛЯЕМ СООБЩЕНИЕ БОТА ЧЕРЕЗ 7 СЕКУНД
    # ============================================================
...
async def cmd_admin_coins(message: types.Message):
    """/coins give @username 10 или /coins take @username 10"""
    ...
    # Если take — делаем отрицательным
    if action == 'take':
        amount = -amount
...
async def handle_phrase_confirm(callback: types.CallbackQuery):
    ...
    # Находим символы для триггера и настроения
    trigger_symbol = None
    mood_symbol = None
    for k, v in TRIGGER_SYMBOLS.items():
        if v == parsed['trigger']:
            trigger_symbol = k
            break
    for k, v in MOOD_SYMBOLS.items():
        if v == parsed['mood']:
            mood_symbol = k
            break
    
    if not trigger_symbol or not mood_symbol:
        await callback.answer("❌ Ошибка: не найден символ", show_alert=True)
        return
    
    success, msg, _ = add_phrase_from_text(f"{trigger_symbol}{mood_symbol} {parsed['text']}")
