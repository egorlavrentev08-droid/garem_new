# test.py - СИСТЕМА ТЕСТИРОВАНИЯ (БЕЗ ЗАВИСИМОСТИ ОТ CONFIG)
...
TIME_MULTIPLIER = 3600  # 1 час = 1 секунда
...
def get_adjusted_seconds(real_seconds: int) -> int:
    """Преобразует реальные секунды в ускоренные (1 час = 1 сек)"""
...
def get_time_since_hours(last_time: datetime) -> float:
    ...
    В тестовом режиме 1 реальный час = 1 секунда.
    """
    from datetime import datetime
    ...
    # В тестовом режиме: реальные секунды / 3600 = ускоренные часы
    # Но мы возвращаем "часы" в ускоренном времени
    # 1 реальная секунда = 1 ускоренный час
